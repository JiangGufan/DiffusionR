...
## inst/benchmarks/image_demo.R
##
## 演示：在 MNIST 上训练 CNN diffusion 模型，并生成样本 + 轨迹
...
run_mnist_diffusion_demo <- function(
  epochs = 5,
  T      = 1000,
  lr     = 1e-4,
  n_samples = 16,
  ...
  cat("Training diffusion CNN on MNIST ...\n")

  schedule_cosine <- beta_cosine(T = T)
  ...
  cat("Sample array dim:", paste(dim(samples), collapse = " x "), "\n")

  # ---- 2. 简单绘制 n_samples 张图片 ----
  op <- par(mfrow = c(4, 4), mar = c(0.1, 0.1, 0.1, 0.1))
  on.exit(par(op), add = TRUE)

  for (i in 1:min(n_samples, 16)) {
    img <- samples[i, , ]
    image(
      1:28, 1:28,
      t(apply(img, 2, rev)),
  ...
  op2 <- par(mfrow = c(length(t_vis), 4), mar = c(0.1, 0.1, 0.1, 0.1))
  on.exit(par(op2), add = TRUE)

  for (j in seq_along(t_vis)) {
    Im <- traj[[j]]
    for (i in 1:4) {  # 每个 t 显示 4 个样本
      img <- Im[i, , ]
      image(
        1:28, 1:28,
        t(apply(img, 2, rev)),
...
# 默认不自动跑，用户可以手动调用：
# res_mnist <- run_mnist_diffusion_demo(epochs = 5)
