...
## inst/test_demo.R
##
## 快速自检脚本：小规模跑一遍 distribution + MNIST，确保核心流程跑通

library(diffuR)
library(torch)
library(torchvision)

cat("===== Quick test: swiss_roll diffusion (tiny) =====\n")

set.seed(1)
torch::torch_manual_seed(1)

X <- swiss_roll(n = 500, noise = 0.1, seed = 1)
Xmean <- colMeans(X)
Xsd   <- apply(X, 2, sd)
X_std <- scale(X, center = Xmean, scale = Xsd)

sch <- beta_cosine(T = 200)
...
cat("Swiss_roll fake_std dim:", paste(dim(fake_std), collapse = " x "), "\n")

cat("\n===== Quick test: MNIST diffusion CNN (tiny) =====\n")
...
cat("MNIST samples_small dim:", paste(dim(samples_small), collapse = " x "), "\n")
cat("MNIST samples_small range:", range(samples_small), "\n")
